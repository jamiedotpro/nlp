...
# 학습에 활용할 텍스트 파일 읽기
train_tmp = read_data('nsmc-master/ratings_train.txt')
test_tmp = read_data('nsmc-master/ratings_test.txt')

def kor_movie(max_num_words=1000):
    # 데이터 구조: ID | 리뷰데이터 | 감성라벨
    # 필요없는 id를 제외하고 리뷰데이터와 감성라벨로만 이루어진 데이터를 만든다.
    # 학습용
    train_x = []
    train_y = []
    ...
    for i in range(len(test_tmp)):
        ...
        test_y.append(test_tmp[i][2])

    # 단어 사전을 만들고, 문장을 단어사전에 맞게 자연수로 변형한다.
    # 빈도수가 높은 단어 순으로 max_num_words개의 단어가 들어있는 사전 생성
    tokenizer = Tokenizer(num_words=max_num_words)
    tokenizer.fit_on_texts(train_x)

    # 위에서 만든 단어사전을 기준으로 리뷰 텍스트 데이터를 자연수의 수열로 바꿔준다.
...
# 각 영화 리뷰 문장들의 길이가 다르다.
# 딥러닝을 하기 위해서는 길이를 통일할 필요가 있다.
# 100 길이 안에서 데이터를 채우고, 길이가 부족해서 빈 영역이 생기는 경우 0으로 채운다.
